...
- 小何自主学习 Agent：右下角原创黑白猫咪悬浮入口，保留手绘原型并带有呼吸、轻摆与点击笑脸动画；支持自由拖动、英语对话、句子批改、单词与语法解释、图片识别和四种界面语言。Agent 可读取真实学习概况与课程、搜索课程、多步规划、创建和编辑课程、更新单词状态、切换页面、导出 PDF/Word，并生成可下载的 PowerPoint 文件
...
小何后端使用 Gemini function calling 选择工具，浏览器负责执行工具并把真实结果返回模型；一次任务最多连续执行四轮工具调用。读取概况、列出课程、读取课程详情和课程搜索会自动执行；创建、修改、删除、学习状态更新、文件导出和 PPT 下载必须先经过浏览器确认。被拒绝的操作不会写入数据。
