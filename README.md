...
- 小何学习助手：右下角原创黑白猫咪悬浮入口、可收起对话框、英语对话、句子批改、单词与语法解释、图片识别、四种界面语言和本地聊天记录
...
仓库根目录的 `render.yaml` 用于在 Render 创建免费 Node.js Web Service。部署时只需在 Render 的密钥输入框中填写 `GEMINI_API_KEY`；不要把密钥写进 `render.yaml`、`js/config.js` 或提交到 GitHub。

部署目标地址为 `https://xiaohe-english-agent-hexin11.onrender.com`，公开网页会自动连接这个 HTTPS 后端。本地开发仍连接 `http://127.0.0.1:8787`。部署完成后可以访问 `/health` 检查 `configured` 和 `capabilities.vision` 是否为 `true`。
...
管理员可在网页中修改公开课程，并从管理台发布到 `site_content`；普通用户打开公共课程时只能学习、点读、收藏和记录进度，不会看到修改或删除入口。每个人仍可使用课时旁边的加号、文件导入功能创建私人课程，私人课程的改名、整课删除和内容增删先保存在 `localStorage`，登录后同步到该账号自己的 `user_snapshots`，不会被其他账号看到。所有删除和全站发布操作都采用二次点击确认。
...
前端仍只使用相对路径，可直接运行在 GitHub Pages。Supabase Auth 与云数据库可以直接从 GitHub Pages 前端访问。GitHub Pages 不能运行 Node.js，所以公开版小何仍需要把 `server/` 单独部署到支持 Node.js 与环境变量的 HTTPS 服务，再把 `js/config.js` 的线上 `agentApiBase` 改为该后端地址。只把域名写进前端，Gemini 密钥仍必须保存在后端平台的环境变量中。
