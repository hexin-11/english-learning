...
- 逐词点读：点击句中单词弹出音标、词性和多条中英文释义，保留原句中文翻译，并可直接选择美式或英式朗读
- 双层单词搜索：课程词库支持中英文搜索，课程外英文可继续查询在线词典
- 在线词典结果：音标、中文直译、多个词性、各词性下的多条中英文释义及美式/英式朗读
- 单词卡：正面显示英文和音标，背面显示课程中文释义、多个词性与对应义项；支持课程筛选、打乱顺序，并自动加载经过语义筛选、带来源标注的单词参考图片，可手动“换一张”并记住选择
...
- 小何自主学习 Agent：右下角原创黑白猫咪悬浮入口，保留手绘原型并带有呼吸、轻摆与点击笑脸动画；支持自由拖动、英语对话、句子批改、单词与语法解释、图片识别和四种界面语言。Agent 可读取真实学习概况与课程、搜索课程、多步规划、创建和编辑课程、更新单词状态、切换页面、导出 PDF/Word，并生成可下载的 PowerPoint 文件；修改后会自动核验结果，同时使用账号隔离的长期学习记忆和课程相关性检索
- 账号与云同步：提供独立注册、密码登录、找回密码和账号设置页面；首次注册验证邮箱后，同一账号会在不同手机和电脑之间自动实时同步聊天、结构化课程、课程修改、收藏和学习进度
...
小何后端使用 Gemini function calling 选择工具，浏览器负责执行工具并把真实结果返回模型；一次任务最多连续执行四轮工具调用。读取概况、列出课程、读取课程详情和课程搜索会自动执行；创建、修改、删除、学习状态更新、文件导出和 PPT 下载必须先经过浏览器确认。被拒绝的操作不会写入数据。修改工具完成后会重新读取本地状态核验结果，核验失败时不会宣称成功。

用户明确说“记住”或表达英语学习目标、发音与讲解偏好时，小何会写入账号隔离的长期学习记忆；登录后该记忆和聊天记录一起进入 Supabase 快照并在设备间同步，退出账号后会从当前页面清除。每次提问还会在浏览器中计算课程相关性，只把匹配的单词与句子片段放入上下文；信息不足时再调用课程详情或搜索工具，避免根据课程标题猜测。

目前可执行的工具包括：

- 学习概况、课程列表、课程详情和课程内容搜索
- 创建课程、修改课名、添加单词或中英句子、删除可编辑课程
- 收藏、取消收藏、标记掌握或待复习
- 打开首页、课程、搜索、收藏、单词卡和拼写页面
- 导出课程 PDF/Word
- 按主题生成黑白简洁风格的 `.pptx` 学习演示并下载；首次使用会从 CDN 按需加载 PptxGenJS

这些能力只在当前英语学习网站和用户已授权的数据范围内执行。小何不会静默控制任意外部网站，也不会自行读取电脑文件、邮箱或其他账号；这类能力需要为每个外部服务单独接入受限工具、身份授权、审计记录和人工确认。长时间任务以后可继续接入 Cloudflare Workflows，使任务支持重试、暂停和人工审批。
...
首次启用账号隔离时，已经登录的原账号会自动接管升级前的本机记录。之后每次登录都会清除访客工作区并读取该账号自己的云端快照；本机修改会自动上传，并通过 Supabase Realtime 通知同账号的其他设备立即检查更新。若实时连接暂时不可用，页面会每 15 秒兜底检查，并在重新联网或切回页面时立即检查。只有两台设备同时修改产生冲突时才需要人工选择版本。退出时会先同步，若同步失败则暂停退出，避免无提示地丢失学习记录。
...
朗读功能基于浏览器 `SpeechSynthesis`。声音列表会自动等待系统载入，也可以点击“刷新”重新读取；可用英语声音取决于操作系统和浏览器，建议使用最新版 Chrome、Edge 或 Safari。未登录状态使用“访客”资料；同一账号在任何设备登录后会恢复自己的头像、网名和学习记录，并持续自动接收其他设备的更新。

课程内容、学习进度和中英文课程搜索都可在本机完成。查询课程外的英文单词时需要联网，输入内容会发送至 [Free Dictionary API](https://dictionaryapi.dev/) 获取音标和英文释义，并发送至 [MyMemory](https://mymemory.translated.net/doc/spec.php) 获取中文直译。单词卡图片通过 [Openverse](https://openverse.org/) 检索，结合课程词义、图片标题和标签进行相关度筛选，并显示作者、许可和来源链接；具体词优先使用直接匹配的实物照片，抽象词使用经过限定的生活情景图，低置信度结果则改用包含中文词义的黑白概念卡，不会强行展示错误图片。候选图片、人工选择和相邻卡片预加载都会保存在当前浏览器中，以减少重复请求和等待。以上服务均无需在项目中配置 API key。
