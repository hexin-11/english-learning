...
未登录时使用独立的访客工作区；登录后会切换到该账号自己的个人资料、聊天、课程与学习记录。退出前网站会先同步最新快照，成功后清除这台设备当前页面中的私人数据并恢复“访客”头像与网名，因此共用设备不会继续显示上一个账号的内容。原始 PDF、Word 和图片不会上传。只有小何服务端需要 `GEMINI_API_KEY`，该密钥不会进入网页代码或 GitHub Pages。
