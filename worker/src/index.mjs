...
9. 先判断版面结构再读文字：表格按“同一行、相邻单元格”配对；双栏页面分别从上到下读取，禁止横跨两栏拼接。重复表头、页眉、页脚只保留一次。
10. 若图片倾斜、透视变形或局部模糊，优先识别清晰区域；不得用相似单词补猜模糊字符。一个词中只要关键字母不确定，就不要加入 words，可在 rawText 中用 [?] 标记。
11. 问题与回答必须保持独立；段落换行只在语法和标点明确连续时合并。不要把上一题结尾与下一题标题合成一句。
