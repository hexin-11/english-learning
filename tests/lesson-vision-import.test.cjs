...
assert.match(importerSource, /MAX_VISION_PDF_PAGES\s*=\s*6/);
assert.match(importerSource, /PDF_EXTRACTION_CONCURRENCY\s*=\s*4/);
assert.match(importerSource, /recognizeScannedPdfWithVision/);
assert.match(importerSource, /mapLimit\(pageNumbers, PDF_EXTRACTION_CONCURRENCY/);
assert.match(importerSource, /SUPPORTED_VISION_TYPES\.has\(file\.type\).*file\.size <= MAX_VISION_IMAGE_BYTES/s);
